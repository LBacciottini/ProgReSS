...
from progress.net.p_module import Module
from progress.net.messages import InterModuleMessage
...
class DAG:
    ...
    the class :class:`~progress.net.kernel.p_module.Module`. The DAG is used to represent the input/output
    relationship between modules, that determines the order in which the modules process the tokens.

    Parameters
    ----------
    nodes : dict[str, :class:`~progress.net.kernel.p_module.Module`]
    ...
    def set_qhal(self, qhal):
        ...
        Parameters
        ----------
        qhal : :class:`~progress.net.kernel.qhal.QHAL`
        """
...
class DAGFactory:
    ...
    :class:`~progress.net.kernel.p_module.ModuleBehavior` classes.
    ...
    Parameters
        ...
        edges : list[tuple[str, str]]
            A list of tuples, where each tuple represents an edge in the DAG. The order of the tuple is important,
            as it determines the direction of the edge.
        module_behaviors : dict[str, tuple(:class:`~progress.net.kernel.p_module.ModuleBehavior.class`, dict)]
    ...
    def create_dag(self):
        ...
        Returns
        -------
        :class:`~progress.net.kernel.dag.DAG`
        """
        modules = {}
        for node in self._module_behaviors.keys():
...
class DAGMessagesSwitch(ns.components.Switch):
    ...
    Parameters
    -----------
    dag : :class:`~progress.net.kernel.dag.DAG`
