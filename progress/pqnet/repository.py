...
class DEJMPSModuleBehavior(ProcessingModuleBehavior):
    ...
    def handle_new_token(self, request):
        ...
        If the module is not the solicitor, it must wait for a message from the solicitor, and just stores the token.
    ...
    def handle_response(self, request):
        r"""
        If the module is the solicitor, it must send the distillation outcome to the solicited module.
        If the module is not the solicitor, it must check whether the distillation was successful and tell
        the solicitor.
    ...
    def handle_message(self, request):
        r"""
        If the module is the solicitor, it receives distillation outcomes from the solicited module.
        If the module is not the solicitor, it receives distillation solicitations from the solicitor.
...
class FreeEverythingModuleBehavior(ProcessingModuleBehavior):
    ...
    def handle_new_token(self, request):
        ...
        See Also
        --------
        :meth:`progress.pqnet.p_module.ProcessingModuleBehavior.handle_new_token`
        """
