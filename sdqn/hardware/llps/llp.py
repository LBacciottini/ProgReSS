...
core services that a link layer protocol must provide to the :class:`~sdqn.components.stack_engine.StackEngine`
...
from sdqn.sockets import Socket
...
class LinkProtocol(ABC, ServiceProtocol):
    ...
    services that a link layer protocol must provide to the QHAL and to progress in general.
    ...
    node : :class:`~sdqn.repeater.Repeater` or None, optional
        ...
        :meth:`~sdqn.repeater.Repeater.subscribe_link_protocol`.
    other_node_info : tuple or None, optional
        ...
        interface (str). This info is used to generate a :class:`~sdqn.sockets.Token` from each
    ...
    1. :class:`~sdqn.mps.protocols.mps_protocol.req_reset`
        Free a qubit for a new entanglement.
    2. :class:`~sdqn.mps.protocols.mps_protocol.req_stop_generation`
        Stop the mid-point entangling source. If already stopped, it has no effect.
    3. :class:`~sdqn.mps.protocols.mps_protocol.req_resume_generation`
    ...
    :class:`~sdqn.mps.protocols.mps_protocol.req_stop_generation` request.
