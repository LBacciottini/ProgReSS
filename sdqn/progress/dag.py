...
from sdqn.progress.p_module import Module
from sdqn.progress.messages import InterModuleMessage
...
class DAG:
    ...
    the class :class:`~sdqn.progress.kernel.p_module.Module`. The DAG is used to represent the input/output
    ...
    def __init__(self, nodes, edges):
        ...
        Parameters
        ----------
        nodes : dict[str, :class:`~sdqn.progress.kernel.p_module.Module`]
    ...
    def set_qhal(self, qhal):
        ...
        Parameters
        ----------
        qhal : :class:`~sdqn.progress.kernel.qhal.QHAL`
        """
...
class DAGFactory:
    ...
    :class:`~sdqn.progress.kernel.p_module.ModuleBehavior` classes.

    Parameters
        ----------
        edges : list[tuple[str, str]]
        module_behaviors : dict[str, tuple(:class:`~sdqn.progress.kernel.p_module.ModuleBehavior.class`, dict)]
    ...
    def create_dag(self):
        ...
        Returns
        -------
        :class:`~sdqn.progress.kernel.dag.DAG`
        """
        modules = {}
        for node in self._module_behaviors.keys():
...
class DAGMessagesSwitch(ns.components.Switch):
    ...
    using the `module_id` as routing key. It also acts as a multiplexer for all outgoing messages from
    bricks in the DAG.

    Parameters
    -----------
    dag : :class:`~sdqn.progress.kernel.dag.DAG`
